...
def parse_arguments():
    parser = argparse.ArgumentParser(description='QSM reconstruction using Rapid Two-Step algorithm')
    ...
    parser.add_argument('--unwrapping', choices=['romeo', 'laplacian'], default='romeo',
                        help='Phase unwrapping algorithm')
    parser.add_argument('--bf_algorithm', choices=['vsharp', 'pdf'], default='vsharp',
                        help='Background field removal algorithm')
    return parser.parse_args()

def main():
    print("[INFO] Starting QSM-RTS algorithm...")
    ...
    unwrapping = args.unwrapping
    bf_algorithm = args.bf_algorithm
    ...
    print(f"[INFO] Unwrapping algorithm: {unwrapping}")
    print(f"[INFO] Background field algorithm: {bf_algorithm}")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Run QSMxT with RTS
        print("[INFO] Running QSMxT with RTS...")
        qsmxt_cmd = [
